...
def main():
    ...
    recordings = ["../data/SDF_UserStudy_Data/Participant_3/2022-11-04 14.32.45_anatE_baseline_P3T1/000"]
    ...
    for rec in recordings:
        ...
        try:
            ...
            process_recording(
                rec, args.out_prefix, overwrite=args.overwrite, history_length_seconds=args.filter_length, 
                onset_confidence_threshold=args.onset, offset_confidence_threshold=args.offset)
...
def process_recording(recording, csv_out_prefix, overwrite=False, **algorithm_kwargs):
    ...
    csv_out_path_class_name = csv_out_path_class.split("/")[-1]
    csv_out_path_interm_name = csv_out_path_interm.split("/")[-1]

    if os.path.exists(csv_out_path_class):
        if not overwrite:
            logger.warning(f"{csv_out_path_class_name} exists already! Not overwriting.")
            return
        else:
            logger.warning(f"{csv_out_path_class_name} exists already! Overwriting.")

    pupil_data = pd.DataFrame(load_and_yield_confidence_data(recording))
    blinks = run_blink_detection(pupil_data, **algorithm_kwargs)
    logger.info(f"Writing intermediate data to {csv_out_path_interm_name}")
    blinks.intermediate.to_csv(csv_out_path_interm, index=False)
    logger.info(f"Writing classification results to {csv_out_path_interm_name}")
